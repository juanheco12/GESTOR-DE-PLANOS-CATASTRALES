...
type Solicitud = {
  ...
  plan: { id: string; radicado: string; predial: string | null };
...
export default function SolicitudesTab() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar esta solicitud? El plano quedará disponible nuevamente si estaba prestado.")) return;
    setDeletingId(id);
    try {
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Elimina solicitudes de prueba o incorrectas. Al eliminar se limpia la bandeja del ejecutor.
          </p>
        </div>
        <button
          onClick={load}
