...
export default async function GestionarSolicitudPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Nombre del receptor asignado (para mostrar en el candado)
  const adminEntrega = solicitud.adminEntregaId
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="font-medium text-slate-900 dark:text-slate-100">
                {solicitud.fechaSolicitud.toLocaleString("es-CO", { timeZone: "America/Bogota", dateStyle: "short", timeStyle: "short" })}
              </p>
            </div>
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400 flex items-center mb-1"><FileText className="mr-2 h-4 w-4" /> Trámite</p>
      ...
      <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700">
        <ActionButtons
          ...
          adminEntregaId={solicitud.adminEntregaId ?? null}
          adminEntregaNombre={adminEntrega?.name ?? null}
